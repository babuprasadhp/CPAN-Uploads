...
'[WARN]: apx_file is not passed, by default your apx file name will be : C:/'
    ...
    while ( my $data = <DATA> ) {
        $data =~ s/\<Symbol\>$text_dummy\<\/Symbol\>/\<Symbol\>$args->{symbol}\<\/Symbol\>/g;
        $data =~ s/\<FormulaPath\>$text_dummy\<\/FormulaPath\>/\<FormulaPath\>$args->{afl_file}\<\/FormulaPath\>/g;
        $data =~ s/\<FormulaContent\>$text_dummy\<\/FormulaContent\>/\<FormulaContent\>$afl\<\/FormulaContent\>/g;
        $data =~ s/\<Periodicity\>$text_dummy\<\/Periodicity\>/\<Periodicity\>$periods{$args->{periodicity}}\<\/Periodicity\>/g;
        $data =~ s/\<FromDate\>$text_dummy\<\/FromDate\>/\<FromDate\>$args->{from}\<\/FromDate\>/g;
        $data =~ s/\<ToDate\>$text_dummy\<\/ToDate\>/\<ToDate\>$args->{to}\<\/ToDate\>/g;
        $data =~ s/\<ApplyTo\>$text_dummy\<\/ApplyTo\>/\<ApplyTo\>$args->{apply_to}\<\/ApplyTo\>/g;
        $data =~ s/\<RangeType\>$text_dummy\<\/RangeType\>/\<RangeType\>$args->{range_type}\<\/RangeType\>/g;
        print $fh $data;
    }
